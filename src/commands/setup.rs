...
//! persists a `BRIGID_MASTER_KEY` in a gitignored `.dev/.env`.
...
fn ensure_mkcert_present() -> Result<()> {
    ...
    bail!(
        "mkcert is not on PATH. It's installed by .devcontainer/setup-container.sh — \
...
fn ensure_web_cert() -> Result<()> {
    let cert_dir = workspace_root().join("web").join(".cert");
    let cert = cert_dir.join("brigid.localhost.pem");
    let key = cert_dir.join("brigid.localhost-key.pem");

    if cert.is_file() && key.is_file() {
        println!("✓ web/.cert/brigid.localhost already present");
        return Ok(());
    }
    ...
    println!("→ generating web/.cert/brigid.localhost cert...");
    ...
    println!("✓ web/.cert/brigid.localhost generated");
    Ok(())
}
...
fn ensure_master_key() -> Result<()> {
    ...
    if let Some(key) = env_file::read_master_key(&env_path) {
        if key.len() == 64 && key.chars().all(|c| c.is_ascii_hexdigit()) {
            println!("✓ .dev/.env BRIGID_MASTER_KEY already present");
            return Ok(());
        }
    }
    ...
    println!(
        "✓ BRIGID_MASTER_KEY written to .dev/.env\n\
