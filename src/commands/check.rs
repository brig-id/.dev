...
fn command_check(label: &str, program: &str, args: &[&str]) -> CheckResult {
    match Command::new(program).args(args).output() {
        ...
        _ => CheckResult {
            label: label.to_string(),
            ok: false,
            detail: format!("`{program}` not found on PATH — see .devcontainer/setup-container.sh"),
        },
    }
}
...
fn web_cert_check() -> CheckResult {
    ...
    CheckResult {
        label: "web/.cert/brigid.localhost".to_string(),
        ok,
        detail: if ok {
            "present".to_string()
        } else {
...
fn master_key_check() -> CheckResult {
    ...
    let label = ".dev/.env BRIGID_MASTER_KEY".to_string();
