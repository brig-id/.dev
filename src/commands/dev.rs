...
pub fn run() -> Result<()> {
    ...
    let include_web = Confirm::new("Include web (Qwik UI)?")
        .with_default(true)
        .prompt()?;
    ...
    if include_web {
        procs.insert("web".to_string(), web_proc());
    }

    let config = MprocsConfig { procs };
    let config_path = dev_root().join("mprocs.yaml");
...
fn web_proc() -> MprocsProc {
    MprocsProc {
        shell: "corepack pnpm dev".to_string(),
        cwd: workspace_root().join("web").display().to_string(),
        env: BTreeMap::new(),
    }
}
